...
#define R(fn) if (strcmp(name, #fn) == 0) return (void *)fn;
    ...
    R(micro_os_ptty_key_input)
    R(micro_os_keyboard_device_input)
    R(micro_os_keyboard_device_subscribe)
    R(micro_os_keyboard_device_unsubscribe)
    ...
    R(micro_os_signal_set_ignored)
    R(micro_os_setpgid)
    R(micro_os_getpgid)
    R(micro_os_tcsetpgrp)
    R(micro_os_tcgetpgrp)
